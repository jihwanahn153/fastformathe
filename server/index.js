...
- CEX: 아주 쉬운 예 하나. 숫자를 넣어 직접 굴려 보인다.
...
CONCEPT 블록은 1개에서 4개 사이로 쓴다. ADVTITLE이 없음이면 하나도 쓰지 않는다.
CONCEPT과 TERM은 짧게 줄이려 하지 마라. 학생이 처음 듣는다는 전제를 끝까지 지켜라.
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/api/solve" && req.method === "POST") {
    try {
      ...
      if (skipAdvanced) {
        instruction +=
          "\n\n이번에는 상위 교육과정 풀이가 필요 없다. ADVTITLE에 없음이라 적고 " +
          "나머지 ADV 칸과 CONCEPT 블록, TERM 칸을 모두 비워라.";
      }
      content.push({ type: "text", text: instruction });

      // 검산까지 하려면 넉넉한 토큰이 필요하다. SCRATCH는 화면에 보이지 않는다.
      // 개념 해부까지 쓰려면 출력이 길다. 상위 풀이를 켠 경우 넉넉히 준다.
      const text = await callAnthropic(content, SOLVE_SYSTEM, skipAdvanced ? 8000 : 24000);
      json(res, 200, { text });
    } catch (e) {
